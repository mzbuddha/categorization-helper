...
def auto_inject_x_templates(df_meta, mapping):
    """
    Automatically adds 'X' templates to df_meta by analyzing label patterns.
    If Column G (flag) is '구조변환 필요', and label ends with a number (e.g. '구분10'),
    and that number exists in the variable name (e.g. 'Q4_1_10_1'),
    it creates/injects an X-template (e.g. 'Q4_1_X_1').
    """
    var_idx = mapping['var']
    label_idx = mapping['label']
    trans_idx = mapping.get('trans', mapping['flag']) # Fallback to flag if trans not present
    
    df_meta_aug = df_meta.copy()
    existing_vars = set(df_meta_aug.iloc[:, var_idx].astype(str).tolist())
    
    detected_templates = {} # template_var -> one_source_row
    
    for i, row in df_meta.iterrows():
        flag_val = str(row.iloc[trans_idx])
        if '구조변환 필요' in flag_val:
            var_name = str(row.iloc[var_idx])
            label = str(row.iloc[label_idx])
            
            # Find trailing number in label (e.g. "구분10" -> "10")
            m_label = re.search(r'(\d+)$', label)
            if m_label:
                num_str = m_label.group(1)
                # Check if this number exists in the var_name
                # More robust: replace the LAST occurrence of the number if it's preceded by _ or is at the end
                if f"_{num_str}" in var_name:
                    template_var = var_name.replace(f"_{num_str}", "_X", 1) # Simple replace for now
                    if template_var not in existing_vars and template_var not in detected_templates:
                        detected_templates[template_var] = row.copy()
                        detected_templates[template_var].iloc[var_idx] = template_var
                elif var_name.endswith(num_str) and len(var_name) > len(num_str):
                    # Handle cases like Q110 where label is "item 10" -> Q1X
                    # But be careful not to replace parts of the name.
                    # Usually, survey data has Q1_1 or Q101.
                    # Let's stick to _ if possible, or if it matches the suffix
                    template_var = var_name[:var_name.rfind(num_str)] + "X"
                    if template_var not in existing_vars and template_var not in detected_templates:
                        detected_templates[template_var] = row.copy()
                        detected_templates[template_var].iloc[var_idx] = template_var
    
    if detected_templates:
        new_rows_df = pd.DataFrame(detected_templates.values())
        df_meta_aug = pd.concat([df_meta_aug, new_rows_df], ignore_index=True)
        
    return df_meta_aug

def detect_transformation_bases(df_meta, mapping):
    """Scans metadata for any variables matching _X_ patterns and returns unique base prefixes."""
    var_idx = mapping['var']
    bases = set()
    ...
    for _, row in df_meta.iterrows():
        ...
        for pat in patterns:
            match = re.match(pat, var_template)
            if match:
                bases.add(match.group(1))
                break
    return sorted(list(bases))

def build_long_dataset_v3(df_resp, df_meta, mapping, set_key, banner_cols, id_col='idx'):
    """
    Metadata-template based Wide to Long transformation.
    Handles various 'X' placeholders for person_no.
    """
    ...
    # Filter banner_cols for actual existence to avoid KeyError
    banner_cols = [c for c in banner_cols if c in df_resp.columns]

    # 1. Identify Templates from Metadata
    templates = []
    # Patterns for the given set_key
    pat_a = rf'^{re.escape(set_key)}_(\d+)_X_(.+)$'
    pat_b = rf'^{re.escape(set_key)}_X_(.+)$'
    pat_c = rf'^{re.escape(set_key)}_X$'

    var_idx = mapping['var']
    label_idx = mapping['label']
    type2_idx = mapping['type2']
    
    type_a, type_b, type_c = False, False, False
    suffix_infos = {} # suffix -> {'label': label, 'role': 'H'/'M' or None}
    
    for _, row in df_meta.iterrows():
        var_template = str(row.iloc[var_idx])
        label = str(row.iloc[label_idx])
        vtype = str(row.iloc[type2_idx])
        
        m_a = re.match(pat_a, var_template)
        if m_a:
            num, sfx = m_a.group(1), m_a.group(2)
            templates.append({'num': num, 'suffix': sfx})
            type_a = True
            # Detect Role
            role = None
            if '시간' in vtype:
                if '(시)' in label: role = 'H'
                elif '(분)' in label: role = 'M'
            suffix_infos[sfx] = {'label': label, 'role': role}
            continue

        m_b = re.match(pat_b, var_template)
        if m_b:
            sfx = m_b.group(1)
            templates.append({'suffix': sfx})
            type_b = True
            # Detect Role
            role = None
            if '시간' in vtype:
        ...
        m_c = re.match(pat_c, var_template)
        if m_c:
            templates.append({'suffix': '_val'}) # Placeholder suffix
            type_c = True
    ...
    # Logic Branch based on template type
    if type_a:
        # Nested: Base_SetNo_PersonNo_Suffix
        ...
        for s_num in set_nums:
            ...
            data_pattern = rf'^{re.escape(set_key)}_{s_num}_(\d+)_(.+)$'
            col_map = {}
            person_nums = set()
            ...
            for c in df_resp.columns:
                ...
                if m:
                    p_no, sfx = m.group(1), m.group(2)
                    if sfx in suffixes:
                        col_map[(p_no, sfx)] = c
                        person_nums.add(p_no)
            
            if not person_nums: continue
            ...
            for p_no in sorted_persons:
                ...
                all_long_dfs.append(temp_df)
                
    elif type_b or type_c:
        # Simple: Base_PersonNo_Suffix or Base_PersonNo
        suffixes = [t['suffix'] for t in templates]
        # Data Pattern
        if type_b: data_pattern = rf'^{re.escape(set_key)}_(\d+)_(.+)$'
        else: data_pattern = rf'^{re.escape(set_key)}_(\d+)$'
        ...
        for c in df_resp.columns:
            ...
            if m:
                p_no = m.group(1)
                sfx = m.group(2) if type_b else '_val'
                if sfx in suffixes:
                    col_map[(p_no, sfx)] = c
                    person_nums.add(p_no)
        ...
        if person_nums:
            ...
            for p_no in sorted_persons:
                ...
                rename_map = {col_map[(p_no, sfx)]: sfx for sfx in suffixes if (p_no, sfx) in col_map}
                temp_df = temp_df.rename(columns=rename_map)
                if type_c and '_val' in temp_df.columns:
                    temp_df = temp_df.rename(columns={'_val': set_key})
    ...
    return final_df, suffix_infos

def compute_time_decimal_and_total_v2(df_long, set_key=None, suffix_infos=None):
    """
    Computes decimal time columns (_S) based on Hour/Minute components.
    If suffix_infos is provided, pairs H/M by matching their labels.
    """
    if suffix_infos:
        # 1. Identify Hour and Minute suffixes present in df
        hours = {s: info for s, info in suffix_infos.items() if info['role'] == 'H' and s in df_long.columns}
        minutes = {s: info for s, info in suffix_infos.items() if info['role'] == 'M' and s in df_long.columns}
        
        paired_m = set()
        for h_sfx, h_info in hours.items():
            h_label_base = h_info['label'].replace('(시)', '').strip()
            # Find matching minute by label
            found = False
            for m_sfx, m_info in minutes.items():
                if m_sfx in paired_m: continue
                m_label_base = m_info['label'].replace('(분)', '').strip()
                if h_label_base == m_label_base:
                    # Pair found!
                    h_val = pd.to_numeric(df_long[h_sfx], errors='coerce')
                    m_val = pd.to_numeric(df_long[m_sfx], errors='coerce').fillna(0)
                    
                    # New column name logic
                    # If set_key is Q1_1_X and sfx is 1, base is Q1_1
                    base_prefix = set_key.replace('_X', '') if set_key else ""
                    # If h_sfx is already descriptive (like 4_1), keep it simple
                    s_col = h_sfx.replace('_1', '_S') if h_sfx.endswith('_1') else f"{h_sfx}_S"
                    
                    df_long[s_col] = h_val + (m_val / 60)
                    df_long[f'flag_{s_col}_range'] = ((df_long[s_col] < 0) | (df_long[s_col] > 24.1)).astype(int)
    ...
    # Legacy Fallback for Q4/Q7 (if not already handled)
    if 'start_S' not in df_long.columns:
    ...
    if 'end_S' not in df_long.columns:
    ...
    s_cols = [c for c in df_long.columns if c.endswith('_S') and c != 'T_S']
    if len(s_cols) >= 2:
        # Try to find 'start' and 'end' keywords or indices
        start_c = next((c for c in s_cols if 'start' in c or '4_S' in c or '시작' in c), None)
        end_c = next((c for c in s_cols if 'end' in c or '5_S' in c or '종료' in c), None)
        
        if start_c and end_c:
            df_long['T_S'] = df_long[end_c] - df_long[start_c]
            mask = df_long['T_S'] < 0
            df_long.loc[mask, 'T_S'] = df_long.loc[mask, 'T_S'] + 24
            df_long['flag_total_negative'] = 0
...
def main():
    ...
    with st.sidebar:
        ...
        trans_col = st.text_input("구조변환 여부 열 (예: G)", "G")
        ...
        if meta_file:
            ...
            try:
                ...
                mapping = {
                    ...
                    'type2': col_to_idx(type2_col), 
                    'type': col_to_idx(type_col),   
                    'flag': col_to_idx(flag_col),
                    'trans': col_to_idx(trans_col)  # 신규: 구조변환 여부 열
                }

                # Auto-inject X templates based on Column G (flag) and Column D (label)
                if df_meta is not None:
                    df_meta = auto_inject_x_templates(df_meta, mapping)
            except Exception as e:
    ...
    with tab_preprocess:
        if not files_uploaded:
            ...
            available_bases = detect_transformation_bases(df_meta, mapping) if df_meta is not None and mapping else []
            # Debug info (temporary)
            if df_meta is not None:
                st.caption(f"Metadata rows: {len(df_meta)}, Detected bases: {', '.join(available_bases) if available_bases else 'None'}")
            
            preprocess_sets = st.multiselect("구조변환 대상 세트 선택", available_bases, default=[])
            
            # --- Time Conversion Option ---
            default_time_calc = False
            if preprocess_sets and df_meta is not None and mapping:
                var_idx = mapping['var']
                type2_idx = mapping['type2']
                for s_key in preprocess_sets:
                    # Match rows where variable starts with s_key followed by _
                    mask = df_meta.iloc[:, var_idx].astype(str).str.contains(rf'^{re.escape(s_key)}_', regex=True, na=False)
                    if df_meta[mask].iloc[:, type2_idx].astype(str).str.contains("시간", na=False).any():
                        default_time_calc = True
                        break
            
            do_time_calc = st.checkbox("시간 데이터 수치화 적용 (h + m/60)", value=default_time_calc)
            ...
            if st.button("전처리 실행"):
                with st.spinner("전처리 중..."):
                    ...
                    for s_key in preprocess_sets:
                        try:
                            # Use V3: Generalized pattern matching
                            res_tuple = build_long_dataset_v3(df_resp_raw, df_meta, mapping, s_key, banner_cols, id_col)
                            if res_tuple is not None:
                                df_res, sfx_infos = res_tuple
                                if df_res is not None and not df_res.empty:
                                    # Apply Time Calculation if selected
                                    if do_time_calc:
                                        df_res = compute_time_decimal_and_total_v2(df_res, s_key, sfx_infos)
                                    
                                    # Apply Renaming based on User Table (Hardcoded defaults for Q4/Q7_4)
                                    if s_key == "Q4":
                                        ren_map = {
                                        ...
                                        # Default renaming for others
                                        s_key_base = s_key.replace('_X', '')
                                        for col in df_res.columns:
                                            if col in ['person_no', 'set_no', 'idx'] or col in banner_cols: continue
                                            if col == '1' and "_X" in s_key:
                                                # Rename single value column to the base name (e.g. Q1_X)
                                                df_res = df_res.rename(columns={col: s_key})
                                            elif col.endswith('_S') or col == 'T_S':
                                                # Q1_1_X and 1_S -> Q1_1_S
                                                new_col = col.replace('1_S', 'S') if col == '1_S' else col
                                                df_res = df_res.rename(columns={col: f"{s_key_base}_{new_col}"})
                                        
                                    results[s_key] = df_res
                                    st.write(f"✅ {s_key}: {len(df_res.columns)}개 컬럼 확보 (배너 포함)")
                        except Exception as e:
            ...
            if "processed_results" in st.session_state:
                ...
                def add_info(base_suffix, label):
                    # Check across all processed result sets
                    for s_key in results.keys():
                        target_df = results[s_key]
                        # For Q4/Q7_4, we know the specific renamed patterns
                        prefix = "Q4_K_X_" if s_key == "Q4" else ("Q7_4_J_X_" if s_key == "Q7_4" else f"{s_key}_")
                        renamed = f"{prefix}{base_suffix}"
                        
                        if renamed in target_df.columns:
                            var_info_list.append([renamed, f"[{s_key}] {label}"])
                            return
                        if base_suffix in target_df.columns:
                            var_info_list.append([base_suffix, f"[{s_key}] {label}"])
                            return
                ...
                if "Q7_4" in results: var_info_list.append(["group_no", "담당 학급 (Q7_4 전용)"])
                var_info_list.append(["person_no", "인력 번호"])
